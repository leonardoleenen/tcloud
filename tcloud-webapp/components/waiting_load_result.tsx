...
const renderStatusSendFile = (props: Props) => {
  switch (props.stage) {
    case WaitingStage.waiting_send_file_response:
      if (props.error_message) return <div className='bg-red-400 rounded-full h-24 w-24  m-auto my-8 flex'><IconCloud /></div>
      return <div className='h-24 w-24  m-auto my-8 flex'> <IconWaiting /></div>
    default:
      return <div className='bg-green-400 rounded-full h-24 w-24  m-auto my-8 flex'><IconCloud /></div>
  }
}
...
const renderStatusConfirmationProcess = (props: Props) => {
  switch (props.stage) {
    case WaitingStage.waiting_send_file_response:
      return <div className='bg-gray-400 rounded-full h-24 w-24  m-auto my-8 flex'><IconClock /></div>
    case WaitingStage.wainting_finish_job:
      if (props.error_message) return  <div className='bg-red-400 rounded-full h-24 w-24  m-auto my-8 flex'><IconClock /></div>
      return <div className='h-24 w-24  m-auto my-8 flex'> <IconWaiting /></div>
    default:
      return <div className='bg-green-400 rounded-full h-24 w-24  m-auto my-8 flex'><IconClock /></div>
  }
}

const labelSendFile = 'Enviando el archivo al servidor para ser procesado por el endpoint:  http://docker01.leafnoise.io:35000/jobs'
...
export default (props: Props) => {
  ...
  return <div className='bg-gray-100 h-screen container'>
    ...
    <div className='bg-white w-3/5 h-3/5 m-auto flex flex-col'>
      ...
      <div className={`m-auto text-4xl my-4 text-center ${props.stage === WaitingStage.waiting_send_file_response ? 'text-gray-500' : 'text-gray-500'}`} > Confirmación de estado </div>
      <div className='m-auto mx-4 text-center'>
        <span className={`m-auto ${textColorConfirmationStatus}`} >Luego del envio se espera confirmación de fin de proceso por parte de server.
</span>
      </div>
    </div>
...
const IconCloud = () => (
  <svg className='m-auto' width="54" height="54" viewBox="0 0 54 54" fill="none" xmlns="http://www.w3.org/2000/svg">
    <g id="Icon/Outline/cloud-upload">
      ...
      <g mask="url(#mask0)">
        <g id="&#240;&#159;&#142;&#168; Color">
          <rect id="Base" width="54" height="54" fill="white" />
        </g>
      </g>
    </g>
  </svg>
...
const IconWaiting = () => (
  ...
  <svg style={{ "margin": "auto", "background": "rgb(255, 255, 255)", "display": "block" }} width="96px" height="96px" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid">
    <circle cx="50" cy="50" r="32" stroke-width="8" stroke="#e15b64" stroke-dasharray="50.26548245743669 50.26548245743669" fill="none" stroke-linecap="round" transform="rotate(258.039 50 50)">
      <animateTransform attributeName="transform" type="rotate" dur="1s" repeatCount="indefinite" keyTimes="0;1" values="0 50 50;360 50 50"></animateTransform>
    </circle>
    <circle cx="50" cy="50" r="23" stroke-width="8" stroke="#667eea" stroke-dasharray="36.12831551628262 36.12831551628262" stroke-dashoffset="36.12831551628262" fill="none" stroke-linecap="round" transform="rotate(-258.039 50 50)">
      <animateTransform attributeName="transform" type="rotate" dur="1s" repeatCount="indefinite" keyTimes="0;1" values="0 50 50;-360 50 50"></animateTransform>
    </circle>
  </svg>
)